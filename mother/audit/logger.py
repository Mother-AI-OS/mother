...
from .redaction import RedactionConfig, get_redactor

# Fields of an AuditEntry that identify or structure the record rather than
# carry content: correlation and session IDs, the event type, the timestamp,
# which capability ran and how it was decided. These are generated by Mother,
# never by a user, so there is nothing in them to redact — and rewriting them
# actively destroys the log's purpose, since a mangled correlation_id cannot be
# joined back to the request it belongs to.
#
# Everything else (params, result, metadata, actor, reason, user_id, source_ip,
# user_agent) is or may contain user data and stays fully redacted.
...
class AuditLogger:
    ...
    def __init__(self, config: AuditLogConfig | None = None):
        ...
        self._redactor = (
            get_redactor(RedactionConfig(preserve_keys=set(_IDENTITY_FIELDS))) if self.config.redact_sensitive else None
        )
